   ...
   /**
    ...
    public static class TrieAutocomplete implements Autocompletor {
        ...
        private void add(String word, double weight) {
            ...
            String input = word;
            double wordWeight = weight;
            Node current = myRoot;
            if (wordWeight > current.mySubtreeMaxWeight){
                current.mySubtreeMaxWeight = wordWeight;
            }
            int len = 0;
            while (len < input.length()) {
                Node temp = new Node(input.charAt(len), current, wordWeight);
                if (!current.children.containsKey(input.charAt(len))) {
                    current.children.put(input.charAt(len), temp);
                }
                current = current.children.get(input.charAt(len));
                if (current.mySubtreeMaxWeight < wordWeight) {
                    current.mySubtreeMaxWeight = wordWeight;
                }
                len++;
            }
            //Once exited the while loop we can assume we are at the end of the word
            current.isWord = true;
            current.setWord(input);
            // Find edge cases here:
            if (current.mySubtreeMaxWeight == current.getWeight()){
                current.setWeight(wordWeight);
                double tempWeight = wordWeight;
                len = 0;
                while (len <= input.length()) {
                    for (Node temp: current.children.values()){
                        if (tempWeight < temp.mySubtreeMaxWeight) {
                            tempWeight = temp.mySubtreeMaxWeight;
                        }
                    }
                    if (tempWeight < current.getWeight()) {
                        tempWeight = current.getWeight();
                    }
                    current.mySubtreeMaxWeight = tempWeight;
        ...
        public Iterable<String> topMatches(String prefix, int k) {
            ...
            PriorityQueue<Node> priority = new PriorityQueue<Node>(new Node.ReverseSubtreeMaxWeightComparator());
            ...
            while (i < prefix.length()) {
                if (current.children.get(prefix.charAt(i)) == null) {
                    return empty;
                }
            ...
            while (priority.size() > 0) {
                ...
                if (terms.size() >= k ) {
                    ...
                    if (terms.get(terms.size() - 1).getWeight() > current.mySubtreeMaxWeight) {
                        break;
                    }
                }
                ...
                if (current.isWord){
                    Term temp = new Term(current.getWord(), current.getWeight());
                    terms.add(temp);
                }
            ...
            Collections.sort(terms, new Term.ReverseWeightOrder()); // Sort final list of words
            //Need to add only as many as k can have

            for (int j = 0; j < k; j++){
                finalWord.add(terms.get(j).getWord());
            }
            return finalWord;
        }
        ...
        public String topMatch(String prefix) {
            ...
            for (int i = 0; i < look.length(); i++) {
                if (current.children.get(look.charAt(i)) == null){
                    return "";
                }
